...
form_security_validate( 'manage_config_message_set' );

auth_reauthenticate();

$t_can_change_level = min( config_get_access( 'plugin_' . plugin_get_current() . 'notify_flags' ), config_get_access( 'plugin_' . plugin_get_current() . 'default_notify_flags' ) );
access_ensure_project_level( $t_can_change_level );

$t_redirect_url = plugin_page( 'manage_config_message_page' );
...
layout_page_header( plugin_lang_get( 'manage_config_message_page' ), $t_redirect_url );

layout_page_begin();

$t_access = current_user_get_access_level();
$t_can_change_flags = $t_access >= config_get_access( 'plugin_' . plugin_get_current() . 'notify_flags' );
$t_can_change_defaults = $t_access >= config_get_access( 'plugin_' . plugin_get_current() . 'default_notify_flags' );
...
if( $t_can_change_defaults ) {
	...
	$t_existing_default_flags = plugin_config_get( 'default_notify_flags' );
	$t_existing_default_access = config_get_access( 'plugin_' . plugin_get_current() . 'default_notify_flags' );
	if( ( $t_existing_default_flags != $t_default_flags )
		...
		plugin_config_set( 'default_notify_flags', $t_default_flags, NO_USER, $t_project, $f_actions_access );
	}
} else {
	$t_default_flags = plugin_config_get( 'default_notify_flags' );
}
...
if( isset( $t_notify_flags ) ) {
	$t_existing_flags = plugin_config_get( 'notify_flags' );
	$t_existing_access = config_get_access( 'plugin_' . plugin_get_current() . 'notify_flags' );
	if( ( $t_existing_flags != $t_notify_flags )
			|| ( $t_existing_access != $f_actions_access ) ) { # only set the flags if they are different
		plugin_config_set( 'notify_flags', $t_notify_flags, NO_USER, $t_project, $f_actions_access );
	}
}

form_security_purge( 'manage_config_message_set' );
