...
layout_page_header( plugin_lang_get( 'manage_config_message_page' ) );
layout_page_begin( 'manage_overview_page.php' );
print_manage_menu( 'manage_plugin_page.php' );
telegrambot_print_menu_config( 'manage_config_message_page' );
...
$t_global_default_notify_flags = plugin_config_get( 'default_notify_flags', null, false, ALL_USERS, ALL_PROJECTS );
...
$g_global_notify_flags = array_merge_recursive2( $g_global_notify_flags, plugin_config_get( 'notify_flags', null, false, ALL_USERS, ALL_PROJECTS ) );

$t_file_default_notify_flags = plugin_config_get( 'default_notify_flags', null, true );
...
$g_file_notify_flags = array_merge_recursive2( $g_file_notify_flags, plugin_config_get( 'notify_flags', null, true ) );

$g_default_notify_flags = plugin_config_get( 'default_notify_flags' );
...
$g_notify_flags = array_merge_recursive2( $g_notify_flags, plugin_config_get( 'notify_flags' ) );

$g_can_change_flags = $t_access >= config_get_access( 'plugin_' . plugin_get_current() . 'notify_flags' );
$g_can_change_defaults = $t_access >= config_get_access(  'plugin_' . plugin_get_current() . 'default_notify_flags' );

echo '<div class="col-md-12 col-xs-12">' . "\n";
echo '<div class="space-10"></div>';

# Email notifications
if( plugin_config_get( 'enable_telegram_message_notification' ) == ON ) {

	if( $g_can_change_flags  || $g_can_change_defaults ) {
		echo '<form id="message_config_action" method="post" action="'. plugin_page( 'manage_config_message_set' ) .'">' . "\n";
		echo form_security_field( 'manage_config_message_set' );
	}
	...
	get_section_begin_for_email( plugin_lang_get( 'manage_config_message_page' ) );
#		get_capability_row_for_email( lang_get( 'email_on_new' ), 'new' );  # duplicate of status change to 'new'
	...
	if( $g_can_change_flags  || $g_can_change_defaults ) {
		...
		print_enum_string_option_list( 'access_levels', config_get_access( 'plugin_' . plugin_get_current() . 'notify_flags' ) );
		echo "\n</select></p>";
		...
		echo '<div>' . "\n";
		echo '<form name="message_config_action" method="post" action="'. plugin_page( 'manage_config_revert') . '">' . "\n";
		...
		echo '<input name="return" type="hidden" value="' . string_attribute( plugin_page( 'manage_config_message_page') ) . '" />' . "\n";
